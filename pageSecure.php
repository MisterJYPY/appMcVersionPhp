                 ...
                 <div class="col-lg-2 col-md-2 col-xs-3 partieimage">
                  ...
                  <div class="col-lg-offset-3 col-md-offset-3 col-xs-offset-4 col-lg-6 miseEnGarde">
                      ...
                      { 
                         ...
                         echo   $sqlg="select comptes_personnel.personnel,personnel.id as id,nom,personnel.mail,personnel.contacts,prenom,fonction,service,services.designation,services.abreviation FROM comptes_personnel,personnel,services
                           ...
                           if(sessionUser::start($login,$passwordCripter, $service->getId(), $perso->getId()))
                                         //completer toutes les variables de SESSION
                                 {
                                   echo 'YESS';
                                     ...
                                     $_SESSION['code_session']=  generateurCode::genererCodeSession();
                                     //if($perso->getFonction()!='RESPONSABLE'){
                                // generateurCode::genererCodeSession();
                                $ip=$_SERVER["REMOTE_ADDR"];
                               $code=$_SESSION['code_session'];
    if($db->insertdata('etatconnection',array(array("personnel"=>$perso->getId(),"ip"=>$ip ,"debut"=>$_SESSION['heure_connection'],"date"=>date('Y-m-d')
          ,"code_session"=>$code )))){
                   header('location:espacePrive.php');
             }
             else{
                 echo '<strong>probleme de Redirection</strong>';
                 }
